...
import { KeyRound, MessageSquare, Loader2, CheckCircle2, ChevronDown, ChevronUp } from 'lucide-react'
...
export function AccountSettings() {
  ...
  async function handleLinkDiscord() {
    ...
    const supabase = createClient()
    // Save current user id before OAuth redirect so we can restore role after
    const { data: { user } } = await supabase.auth.getUser()
    if (user) {
      sessionStorage.setItem('syntra_pre_discord_uid', user.id)
    }
        ...
        <div className="mt-5 space-y-3">
          ...
          >
            <div>
              ...
              <p className="mt-0.5 text-xs" style={{ color: 'rgba(255,255,255,0.35)', fontWeight: 300 }}>
                Connect Discord to unlock community perks
              </p>
            </div>
            <button
              type="button"
