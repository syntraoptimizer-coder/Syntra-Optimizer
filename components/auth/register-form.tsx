...
export function RegisterForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // Optionally store extra profile data in a public profiles table.
    if (data.user) {
      await supabase
        .from('profiles')
        .upsert({ id: data.user.id, full_name: values.name.trim(), email: values.email })
    }
  ...
  if (status === 'success') {
    return (
      <div className="rounded-xl border border-primary/30 bg-primary/10 p-6 text-center">
        ...
        <h2 className="mt-3 font-medium">Account created</h2>
        <p className="mt-1 text-sm text-muted-foreground">
          Welcome to Syntra, {values.name.split(' ')[0]}! Check your inbox to verify your email.
        </p>
      </div>
    )
  }
          ...
          <label className="flex items-start gap-2.5 text-sm">
            <input
              ...
              className={cn(
                ...
                errors.terms && 'outline outline-2 outline-destructive',
              )}
            />
